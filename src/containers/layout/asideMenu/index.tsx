...
const AsideMenu = () => {
  ...
  const getClassBorderBottom = (path: string) => {
    if (location.pathname === path) return 'tw-mt-auto tw-w-full tw-h-[1px] tw-bg-secondary';
  ...
  const getClassTitle = (path: string) => {
    if (location.pathname === path) return 'tw-pb-2 tw-text-white';
    return 'tw-pb-2 tw-text-white';
  };
  const navigateItems = (navigate: Navigate, index: number) => {
    return (
      <Link to={navigate.path} key={index}>
        ...
        >
          <div className={getClassTitle(navigate.path)}>{navigate.title}</div>
    ...
    <div className="tw-h-[100vh] tw-p-4 tw-space-y-4 tw-font-mono tw-shadow-2xl">
      ...
      <div className="">
        {Navigation.map((navigate, index) => {
          return navigate.isAuth && navigateItems(navigate, index);
        })}
      </div>
    </div>
  );
